...
df = pd.read_csv(path+"\\df_sample.csv", index_col=0)
...
DT_res=Image.open(path+'\\DT_res.png')
st.image(DT_res)
...
st.write("On a esnuite réalisé un test avec l'algorithme RandomForestRegressor).")
...
RF_res=Image.open(path+'\\RF_res.png')
st.image(RF_res)
...
st.write("On a finalement fait un test avec l'algorithme des K-plus-proche-voisins (K-NN : KNeighborsRegressor).")
...
st.write("Les résultats obtenus sont les suivants : ")
KNN_res=Image.open(path+'\\KNN_res.png')
st.image(KNN_res)
...
niveau_emission=Image.open(path+'\\niveau_emission.jpg')
st.image(niveau_emission)

st.write("Les résultats obtenus avec l'algorithme K-NN ont été utilisés pour déterminer le niveau d'émission de chaque véhicule du jeu de données")
...
classif=Image.open(path+'\\classif.png')
st.image(classif)

st.write("Les résultats obtenus sont très satisfaisants, le modèle K-NN mis au point est donc efficace pour une classification multiclasse en niveau d'émissions")
...
st.write("Encore une fois avec l'algorithme K-NN, nous avons réalisé la classification des véhicules pour répondre à la question : 'Ce véhicule sera-t-il autorisé à la vente en 2030 ?'")
st.write("Les classes sont donc Oui et Non")

st.write("Les résultats obtenus sont les suivants : ")
loi_climat=Image.open(path+'\\loi_climat.png')
st.image(loi_climat)
