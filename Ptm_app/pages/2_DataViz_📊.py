...
st.pyplot(ft_box)
st.write("Comme on pouvait s'y attendre, ce sont les véhicules Hybrides qui émettent le moins de CO2 au km.")
st.write("Les véhicules roulant à l'E85 sont,quant à eux, ceux qui émettent le plus de CO2 au km.")


st.write("Voici une infographie permettant de visualiser le niveau moyen d'émission de CO2 par groupe automobile : ")
infographie=Image.open(path+'\\infographie_ptm.png')
...
selected_columns = st.multiselect('Selection variables matrice corrélation', columns)
...
def create_plot(df):
    ...
    maker = st.selectbox('Select a car maker', makers)
